...
def _decompose_payload(task_id: str, body: dict, *, board=None):
    """Decompose a triage task into child tasks via auxiliary LLM."""
    from hermes_cli import kanban_decompose as decomp

    author = body.get("author") or "webui"
    try:
        outcome = decomp.decompose_task(task_id, author=author, board=board)
        return {
            "ok": outcome.success,
        ...
        logger.exception("failed to decompose task %s", task_id)
        return {"ok": False, "error": str(exc), "read_only": False}
...
def handle_kanban_post(handler, parsed, body) -> bool | None:
    ...
    try:
        ...
        if path.startswith(_TASK_PREFIX) and path.endswith("/decompose"):
            task_id = path[len(_TASK_PREFIX):-len("/decompose")].strip("/")
            return j(handler, _decompose_payload(task_id, body, board=board)) or True
