...
function toggleTheme() {
  const current = localStorage.getItem('hermes-theme') || 'dark';
  let next;
  ...
  if (typeof _applyTheme === 'function') _applyTheme(next);
  syncThemeIcons();
  
  // Sync the settings picker if open
  const picker = document.getElementById('settingsTheme');
  if (picker) picker.value = next;
  if (typeof _syncThemePicker === 'function') _syncThemePicker(next);
  
  // Persist to server
  api('/api/settings', { method: 'POST', body: JSON.stringify({ theme: next }) }).catch(() => {});
}
...
function syncThemeIcons() {
  ...
  const systems = document.querySelectorAll('.system-icon');
...
function toggleFocusMode() {
  ...
  localStorage.setItem('hermes-focus-mode', isFocus ? '1' : '0');
